<template>
  <div class="relative">
    <svg width="19" height="22" viewBox="0 0 19 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      <mask
        id="mask0_418_744"
        style="mask-type: luminance"
        maskUnits="userSpaceOnUse"
        x="0"
        y="0"
        width="19"
        height="18"
      >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M9.15527e-05 0H18.497V17.348H9.15527e-05V0Z"
          fill="white"
        />
      </mask>
      <g mask="url(#mask0_418_744)">
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M9.2471 1.5C5.75211 1.5 3.3161 4.238 3.3161 6.695C3.3161 8.774 2.7391 9.735 2.2291 10.583C1.8201 11.264 1.4971 11.802 1.4971 12.971C1.6641 14.857 2.90911 15.848 9.2471 15.848C15.5501 15.848 16.8341 14.813 17.0001 12.906C16.9971 11.802 16.6741 11.264 16.2651 10.583C15.7551 9.735 15.1781 8.774 15.1781 6.695C15.1781 4.238 12.7421 1.5 9.2471 1.5ZM9.2471 17.348C4.5711 17.348 0.345105 17.018 0.000104907 13.035C-0.00289509 11.387 0.500105 10.549 0.944105 9.811C1.3931 9.063 1.8161 8.358 1.8161 6.695C1.8161 3.462 4.8021 0 9.2471 0C13.6921 0 16.6781 3.462 16.6781 6.695C16.6781 8.358 17.1011 9.063 17.5501 9.811C17.9941 10.549 18.4971 11.387 18.4971 12.971C18.1481 17.018 13.9231 17.348 9.2471 17.348Z"
          :fill="active ? 'var(--theme-primary)' : 'var(--theme-gray-subtext)'"
        />
      </g>
      <path
        fill-rule="evenodd"
        clip-rule="evenodd"
        d="M9.1983 21.5H9.1963C8.0753 21.499 7.0143 21.005 6.2093 20.108C5.9323 19.801 5.9573 19.326 6.2653 19.05C6.5733 18.772 7.0473 18.797 7.3243 19.106C7.8423 19.683 8.5073 20 9.1973 20H9.1983C9.8913 20 10.5593 19.683 11.0783 19.105C11.3563 18.798 11.8303 18.773 12.1373 19.05C12.4453 19.327 12.4703 19.802 12.1933 20.109C11.3853 21.006 10.3223 21.5 9.1983 21.5Z"
        :fill="active ? 'var(--theme-primary)' : 'var(--theme-gray-subtext)'"
      />
    </svg>
    <div
      v-if="active"
      class="w-[10px] h-[2px] bg-theme-primary rounded-full mx-auto absolute left-1/2 -translate-x-1/2 -bottom-2"
    ></div>
  </div>
</template>

<script setup>
defineProps({
  active: {
    type: Boolean,
  },
})
</script>
